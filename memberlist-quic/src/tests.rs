...
mod unit_tests {
  ...
  fn transport_error_classifies_remote_failures() {
    ...
    assert!(
      QuicTransportError::<TestResolver>::ConnectionClosed("closed".into()).is_remote_failure()
    );
    assert!(QuicTransportError::<TestResolver>::StreamClosed("closed".into()).is_remote_failure());
    assert!(QuicTransportError::<TestResolver>::PacketTooLarge.is_remote_failure());
    ...
    assert_eq!(
      format!("{:?}", QuicTransportError::<TestResolver>::PacketTooLarge),
      "packet too large"
    );
  }
