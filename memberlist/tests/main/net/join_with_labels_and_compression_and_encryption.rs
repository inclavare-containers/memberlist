...
pub async fn memberlist_join_with_labels_and_compression_and_encryption<F, T, R>(
  mut get_transport: impl FnMut(usize, Label, CompressAlgorithm, SecretKey) -> F,
) where
  ...
  R: Runtime,
{
  ...
  let m1 = Memberlist::<T, _>::new(
    ...
    Options::lan()
      ...
      .with_secret_keys(TEST_KEYS.into()),
  )
  .await
  .unwrap();
  ...
  let m4 = Memberlist::<T, _>::new(
    get_transport(4, label.clone(), CompressAlgorithm::default(), TEST_KEYS[0]).await,
    Options::lan()
      .with_label(label)
      .with_compress_algo(CompressAlgorithm::default())
      .with_primary_key(TEST_KEYS[0])
    ...
    paste::paste! {
      ...
      fn [< test_ $rt:snake _ $kind:snake _join_with_labels >]() {
        [< $rt:snake _run >](async move {
          memberlist_join_with_labels_and_compression_and_encryption::<_, NetTransport<_, SocketAddrResolver<[< $rt:camel Runtime >]>, _, [< $rt:camel Runtime >]>, _>(|idx, _, _, _| async move {
